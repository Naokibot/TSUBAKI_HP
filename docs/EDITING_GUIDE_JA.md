...
このサイトはNode.jsで静的HTMLを生成します。UIは装飾を抑え、濃紺と生成りを基調にした落ち着いた構成です。
...
| ブログ記事の新規作成 | `npm run blog:new` |
| ブログ本文 | `content/blog/`内のMarkdownファイル |
...
ブログは長文を書きやすいMarkdown形式です。JSONを直接編集する必要はありません。

```bash
npm run blog:new
```

タイトル、公開日、短い説明、URL名を入力すると、`content/blog/`に記事ファイルが作成されます。作成されたファイルを開き、`---`より下へ本文を書きます。
...
`npm run dev`や公開用ビルドを実行すると、Markdownから`content/posts.json`が自動生成されます。詳しい手順は`docs/BLOG_GUIDE_JA.md`を確認してください。
